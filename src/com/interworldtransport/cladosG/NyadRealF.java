...
public class NyadRealF extends NyadAbstract
{
	...
	public static boolean isMEqual( NyadRealF pTs, NyadRealF pN )
	{
		...
		if ( pTs.getFootPoint() != pN.getFootPoint() )
		{
			return false;
		}
		...
		for ( MonadRealF tSpot : pTs.getMonadList() )
		{
			...
			// if check is true a match was found and it is time to move to next
			// monad in first nyad
			// if check is false, we have a dangling monad, thus nyads can't be
			// equal.
			if ( !forwardCheck )
			{
				return false;
	...
	public static boolean isStrongReferenceMatch( NyadRealF pTs, NyadRealF pN )
	{
		...
		for ( MonadRealF tSpot : pTs.getMonadList() )
		{
			...
			for ( MonadRealF tSpot2 : pN.getMonadList() )
			{
				...
				// TODO Probably should NOT be using equals comparison. Use my own.
	...
	throws FieldBinaryException
	{
		...
		if ( tF >= 0 && tF <= monadList.size() )
		{
			monadList.get( tF ).scale( pMag );
		}
		return this;
	}
