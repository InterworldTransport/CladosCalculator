...
public final class Basis
{
	...
	public String toXMLString()
	{
		...
		{
			rB.append( "\t\t<Grade number=\"" + k + "\" range=\"" + getGradeRange( k ) + "-" + ( getGradeRange( ( short )( k + 1 ) ) - 1 ) + "\" />\n" );
		}
		rB.append( "\t\t<Grade number=\"" + ( getGradeCount() - 1 ) + "\" range=\"" + getGradeRange( ( short )( gradeCount - 1 ) ) + "-" + getGradeRange( ( short )( gradeCount - 1 ) ) + "\" />\n" );
		rB.append( "\t</Grades>\n" );

		rB.append( "\t<Blades count=\"" + getBladeCount() + "\">\n" );
		for ( short k = 0; k < bladeCount; k++ )	// Appending blades
		{
			rB.append( "\t\t<Blade number=\"" + ( k + 1 ) + "\" key=\"" + getBasisKey( k ) + "\" generators=\"" );
